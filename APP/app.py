...
# 2. SCRIPT JAVASCRIPT EDITADO (Paleta adaptable para tema claro y oscuro)
js_color_script = f"""
<script>
function applyEssenceColors() {{
    ...
    // Reglas de color adaptadas según el tema activo
    const colorRules = [
        // Rojo Sangre / Rosas
        {{ keywords: ['sangre', 'cereza', 'frambuesa', 'pimienta rosa', 'rosa', 'ruibarbo', 'lichi'], 
          bgLight: 'rgba(254, 226, 226, 0.95)', borderLight: '#f87171', textLight: '#991b1b',
          bgDark: 'rgba(153, 27, 27, 0.35)', borderDark: 'rgba(220, 38, 38, 0.65)', textDark: '#ededed' }},
        
        // Marinas / Acuáticas
        {{ keywords: ['marina', 'marinas', 'agua', 'océano'], 
          bgLight: 'rgba(207, 250, 254, 0.95)', borderLight: '#22d3ee', textLight: '#0e7490',
          bgDark: 'rgba(14, 116, 144, 0.35)', borderDark: 'rgba(6, 182, 212, 0.65)', textDark: '#ededed' }},
        
        // Herbal / Verde
        {{ keywords: ['albahaca', 'bergamota', 'cardamomo', 'higo', 'manzana', 'menta', 'pachulí', 'pera', 'romero', 'salvia', 'té verde', 'vetiver'], 
          bgLight: 'rgba(220, 252, 231, 0.95)', borderLight: '#4ade80', textLight: '#15803d',
          bgDark: 'rgba(21, 128, 61, 0.35)', borderDark: 'rgba(34, 197, 94, 0.65)', textDark: '#ededed' }},
        
        // Ámbar / Naranja
        {{ keywords: ['azafrán', 'ámbar', 'mandarina', 'melocotón', 'mirra', 'naranjo', 'pomelo'], 
          bgLight: 'rgba(255, 237, 213, 0.95)', borderLight: '#fb923c', textLight: '#c2410c',
          bgDark: 'rgba(194, 65, 12, 0.35)', borderDark: 'rgba(249, 115, 22, 0.65)', textDark: '#ededed' }},
        
        // Cítricos / Amarillo
        {{ keywords: ['caramelo', 'cítrico', 'cítricos', 'jengibre', 'limón', 'miel', 'solares', 'piña', 'vainilla', 'ylang'], 
          bgLight: 'rgba(254, 240, 138, 0.95)', borderLight: '#facc15', textLight: '#854d0e',
          bgDark: 'rgba(161, 98, 7, 0.35)', borderDark: 'rgba(234, 179, 8, 0.65)', textDark: '#ededed' }},
        
        // Florales Violeta / Lavanda
        {{ keywords: ['ciruela', 'grosella', 'grosellas', 'iris', 'lavanda'], 
          bgLight: 'rgba(243, 232, 255, 0.95)', borderLight: '#c084fc', textLight: '#6b21a8',
          bgDark: 'rgba(126, 34, 206, 0.35)', borderDark: 'rgba(168, 85, 247, 0.65)', textDark: '#ededed' }},
        
        // Café / Cacao
        {{ keywords: ['cacao', 'café', 'canela', 'tonka', 'nuez moscada', 'praliné', 'tabaco'], 
          bgLight: 'rgba(245, 230, 211, 0.95)', borderLight: '#b45309', textLight: '#451a03',
          bgDark: 'rgba(88, 28, 12, 0.45)', borderDark: 'rgba(180, 83, 9, 0.75)', textDark: '#ededed' }},
        
        // Resinas / Incienso
        {{ keywords: ['ámbar gris', 'incienso'], 
          bgLight: 'rgba(241, 245, 249, 0.95)', borderLight: '#94a3b8', textLight: '#334155',
          bgDark: 'rgba(71, 85, 105, 0.35)', borderDark: 'rgba(148, 163, 184, 0.65)', textDark: '#ededed' }},
        
        // Maderas / Cuero
        {{ keywords: ['abedul', 'cedro', 'sándalo', 'civeta', 'cuero', 'oud', 'pimienta negra'], 
          bgLight: 'rgba(226, 232, 240, 0.95)', borderLight: '#64748b', textLight: '#1e293b',
          bgDark: 'rgba(55, 65, 81, 0.45)', borderDark: 'rgba(156, 163, 175, 0.65)', textDark: '#ededed' }},
        
        // Almizcle / Neutro
        {{ keywords: ['almizcle', 'coco', 'jazmín', 'nardos', 'neroli', 'pimienta blanca'], 
          bgLight: 'rgba(248, 250, 252, 0.95)', borderLight: '#cbd5e1', textLight: '#0f172a',
          bgDark: 'rgba(255, 255, 255, 0.12)', borderDark: 'rgba(255, 255, 255, 0.3)', textDark: '#ededed' }}
    ];
    ...
    targets.forEach(el => {{
        ...
        for (const rule of colorRules) {{
            if (rule.keywords.some(kw => text.includes(kw))) {{
                const bg = isDark ? rule.bgDark : rule.bgLight;
                const border = isDark ? rule.borderDark : rule.borderLight;
                const color = isDark ? rule.textDark : rule.textLight;
                ...
                el.addEventListener('mouseenter', () => {{ el.style.backgroundColor = border; el.style.color = '#ffffff'; }});
                el.addEventListener('mouseleave', () => {{ el.style.backgroundColor = bg; el.style.color = color; }});
                break;
            }}
        }}
    }});
...
# 3. CSS GLOBAL MINIMALISTA Y SOPORTE DE TEMA CLARO EN POPOVER / SELECTS
st.markdown(f"""
    <style>
    ...
    * {{ font-family: 'Inter', sans-serif !important; }}
    ...
    /* INPUTS, POPOVER Y SELECTS SUTILES CON SOPORTE COMPLETO DE TEMA */
    ...
    div[data-baseweb="input"] input::placeholder {{ color: {subtext_color} !important; opacity: 0.7; }}

    /* CONTENEDOR FLOTANTE POPOVER Y DESPLEGABLES EN MODO CLARO/OSCURO */
    div[data-testid="stPopoverBody"], div[data-baseweb="popover"], div[data-baseweb="menu"], ul[role="listbox"] {{
        background-color: {input_bg} !important;
        border: 1px solid {input_border} !important;
        color: {input_text} !important;
